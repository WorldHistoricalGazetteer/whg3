        ...
        for pid in pids:
            ...
            if role == 'parent':
              ...
              ctx._source.searchy.addAll(params.sugs);",
                    ...
                    print('parent w/kids ' + hit['_source']['title'],
                          pid + ' transferred resp to: ' + parent + '; tagged for deletion')
                    delthese.append(pid)
            elif role == 'child':
                # get its parent
                ...
                ctx._source.suggest.input = params.sugs; ctx._source.searchy = params.searchy;",
                    ...
                    try:
                        es.update_by_query(index=idx, body=q_update)
                        print('child ' + psrc['title'],
                              pid + ' excised from parent: ' + parent + '; tagged for deletion')
                    except:
                        print('aw shit', sys.exit(sys.exc_info()))
