...
class Command(BaseCommand):
    help = 'Migrate place_name from JSONL file'
            ...
            for batch_index, batch in enumerate(batch_iterator(reader, 1000), start=1):
                ...
                for row_index, row in enumerate(batch, start=1):
                    ...
                    try:
                        ...
                        placename, created = PlaceName.objects.get_or_create(
                            place=place,
                            src_id=row['src_id'],
                            jsonb=row['jsonb'],
                            task_id=row['task_id'],
                            ...
                            create_date=row.get('create_date', None),
                        )

                        if created:
                            self.stdout.write(self.style.WARNING(f'Created PlaceName instance for row {row_index}'))
                            placenames.append(placename)  # Append the new PlaceName to the list
                            count += 1
                        else:
                            self.stdout.write(self.style.WARNING(f'Skipped duplicate PlaceName for row {row_index}'))
