...
class Command(BaseCommand):
    help = "Reprocess PlaceGeoms for Dataset 838: add granularity, remove certainty"
    ...
    def handle(self, *args, **options):
        ...
        for pg in placegeoms:
            ...
            pid = pg.place.id
            granularity_value = 30 if pid in SPECIAL_PIDS else 5
            ...
            if geom_type == 'GeometryCollection':
                ...
                for geometry in geom_data.get('geometries', []):
                    ...
                    # Remove 'certainty' if present
                    if 'certainty' in geometry:
                        del geometry['certainty']
                        self.stdout.write(f"    Removed 'certainty' property")
                    # Set granularity
                    geometry['granularity'] = granularity_value
                    self.stdout.write(f"    Set 'granularity' to {granularity_value}")
                    updated = True
            else:
                # Single geometry
                if 'certainty' in geom_data:
                    del geom_data['certainty']
                geom_data['granularity'] = granularity_value
                updated = True
